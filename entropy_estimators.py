...
def kldiv(x, xp, k=3, base=2, return_pw = False):
    ...
    if return_pw == False:
        ...
        point_wise = (const + d * np.array(map(log, nnp)) - d * np.array(map(log, nn))) / log(base);
        return (const + d * np.mean(map(log, nnp)) - d * np.mean(map(log, nn))) / log(base), point_wise;
